...
class TestUtils(PINNTestCase):
    ...
    def test_boundary_sampling_count(self):
        ...
        n = 400
        x, y, u, normals = generate_boundary_data(n, device=self.device)
        self.assertEqual(len(x), n)
        self.assertEqual(len(y), n)
        self.assertEqual(u.shape, (n, 1))
        self.assertEqual(normals.shape, (n, 2))

    def test_boundary_sampling(self):
        """Verifies boundary points are actually on the edges."""
        x_bc, y_bc, _, _ = generate_boundary_data(100, device=self.device)
    ...
    def test_boundary_values_range(self):
        ...
        _, _, u_bc, _ = generate_boundary_data(200, device=self.device)
    ...
    def test_boundary_distribution(self):
        ...
        n_points = 400
        x_bc, y_bc, _, _ = generate_boundary_data(n_points, device=self.device)
        expected = n_points // 4
